...
[![Latest NPM release](https://img.shields.io/npm/v/@3dverse/livelink-camera-controls.svg)](https://www.npmjs.com/package/@3dverse/livelink-camera-controls)
...
```javascript
import CameraControls, { Clock } from 'camera-controls';

const clock = new Clock();
const cameraControls = new CameraControls( cameraEntity.local_transform, cameraEntity.perspective_lens, renderer.domElement );
...
cameraControls.addEventListener("update", () => {
	cameraControls.position.toArray(cameraEntity.local_transform.position);
	cameraControls.orientation.toArray(cameraEntity.local_transform.orientation);
});
```

## Constructor

`CameraControls( localTransform, lens, domElement )`

- `localTransform` is a `Components.LocalTransform` for camera position
- `lens` is a `Components.PerspectiveLens` or `Components.OrthographicLens` for camera projection
...
#### `fitToBox( aabb, enableTransition, { paddingTop, paddingLeft, paddingBottom, paddingRight } )`
...
| Name                    | Type                         | Description |
| ----------------------- | ---------------------------- | ----------- |
| `aabb`                  | `THREE.Box3`                 | Axis aligned bounding box to fit the view. |
| `enableTransition`      | `boolean`                    | Whether to move smoothly or immediately |
...
| Name               | Type                           | Description |
| ------------------ | ------------------------------ | ----------- |
| `sphere`     		 | `THREE.Sphere`                 | bounding sphere to fit the view. |
| `enableTransition` | `boolean`                      | Whether to move smoothly or immediately |
